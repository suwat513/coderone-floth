...
class Agent:
    ...
    def get_bomb_range_tiles(self, board, tile):
        if tile[1] > 0:
    ...
    def update_state_dict(self, board, position):
        if position in self.bomb_dict:
            if self.bomb_dict[position] - 1 <= 0:
                for tile in self.get_bomb_range_tiles(board, position):
                    if tile in self.bomb_dict:
                        self.bomb_dict[tile] = min(1, self.bomb_dict[tile])
    ...
    def verify_state_dict(self, position):
        if position not in self.ore_dict:
            self.ore_dict[position] = 1
    ...
    def strategy1(self, board, player_state, impassable_positions):
        ...
        for x in range(self.cols):
            for y in range(self.rows):
                ...
                if board[position] == 5 and position in self.bomb_dict:
                    for tile in self.get_bomb_range_tiles(board, position):
                        if board[tile] in self.not_block:
                            for i in range(-10 + player_state.hp, 11 - player_state.hp, 1):
    ...
    def next_move(self, game_state, player_state):
        ...
        for bomb in game_state.bombs:
            self.update_state_dict(board, bomb)
        for ore_block in game_state.ore_blocks:
            self.verify_state_dict(ore_block)
        ...
        if self.prev_position and self.prev_action and not self.lag:
            ...
            if not self.prev_action:
                self.lag = False
            return self.prev_action
        # if game_state.tick_number - self.prev_tick < 2:
        #     return ""
        # self.print_map(game_state)
